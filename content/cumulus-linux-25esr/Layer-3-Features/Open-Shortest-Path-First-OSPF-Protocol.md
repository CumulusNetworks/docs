...
As discussed in [Introduction to Routing Protocols](/cumulus-linux-25esr/Layer-3-Features/Introduction-to-Routing-Protocols),
...
3.  Routes, with `show ip ospf route`. This is the outcome of SPF
    ...
    Compare the route output with kernel by using 
    `show ip route | grep zebra` and with the hardware entries using 
    `cl-route-check -V`.
