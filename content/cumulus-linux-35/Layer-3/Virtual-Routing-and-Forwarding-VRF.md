...
characteristics:
  ...
  - Applications can use existing interfaces to operate in a VRF context
    ...
    [management VRF](/cumulus-linux-35/Layer-3/Management-VRF) 
    is enabled,
  ...
  - Listen sockets used by services are VRF-global by default unless the
    ...
    read about [services in the management VRF](/cumulus-linux-35/Layer-3/Management-VRF/#running-services-within-the-management-vrf).
...
interfaces. Keep in mind the following for a VRF table:
  ...
  - Names for VRF tables can be up to 15 characters. However, you
    **cannot** use the name *mgmt*, as this name can **only** be used
    for [management
    VRF](/cumulus-linux-35/Layer-3/Management-VRF).
...
above. The [management VRF chapter](/cumulus-linux-35/Layer-3/Management-VRF/#running-services-within-the-management-vrf) 
...
[OSPFv2](/cumulus-linux-35/Layer-3/Open-Shortest-Path-First-OSPF-Protocol)
and [static routing](/cumulus-linux-35/Layer-3/Routing) (IPv4
...
[BFD](/cumulus-linux-35/Layer-3/Bidirectional-Forwarding-Detection-BFD),
...
external LSAs) and types 9 through 11 (opaque LSAs) link state advertisements, 
redistributing other routing protocols, connected and static routes, and route 
maps. As with BGP, you can track OSPF neighbors with
[BFD](/cumulus-linux-35/Layer-3/Bidirectional-Forwarding-Detection-BFD).
...
FRRouting too, but they become active only when configured with NCLU.
  ...
  - An OSPFv2 instance can be configured using the 
    `net add ospf vrf <VRF>` command; as with BGP, all OSPFv2 parameters 
    can be configured.
...
[BGP unnumbered interface configurations](/cumulus-linux-35/Layer-3/Border-Gateway-Protocol-BGP)
...
name of the service - such as `dhcpd` or `dhcrelay` - and `<vrf-name>` 
is the name of the VRF.
...
## Caveats and Errata
  ...
  - You cannot configure 
    [EVPN address families](/cumulus-linux-35/Network-Virtualization/Ethernet-Virtual-Private-Network-EVPN)
    within a VRF.
