...
3.  If you plan on accessing your switch's BIOS over the serial console,
    ...
    information, see {{<kb_link url="knowledge-base/Configuration-and-Usage/Administration/Accessing-the-BIOS-on-an-x86-Switch/" text="this knowledge base article">}}.
...
File](/cumulus-linux-36/Monitoring-and-Troubleshooting/Understanding-the-cl-support-Output-File/).
...
| 20-clagd.conf     | Logs `clagd` messages to `/var/log/clagd.log` for [MLAG](/cumulus-linux-36/Layer-2/Multi-Chassis-Link-Aggregation-MLAG).                                       |
...
| 30-ptmd.conf      | Logs `ptmd` messages to `/var/log/ptmd.log` for [Prescription Topology Manager](/cumulus-linux-36/Layer-1-and-Switch-Ports/Prescriptive-Topology-Manager-PTM). |
| 35-rdnbrd.conf    | Logs `rdnbrd` messages to `/var/log/rdnbrd.log` for [redistribute neighbor](/cumulus-linux-36/Layer-3/Redistribute-Neighbor).                                    |
| 40-netd.conf      | Logs `netd` messages to `/var/log/netd.log` for [NCLU](/cumulus-linux-36/System-Configuration/Network-Command-Line-Utility-NCLU/).                             |
...
VRF](/cumulus-linux-36/Layer-3/Management-VRF) enabled by
