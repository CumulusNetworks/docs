...
In Cumulus Linux, the BGP `maximum-paths` setting is enabled, so multiple routes are installed by default. See {{<link url="Optional-BGP-Configuration#ecmp" text="ECMP">}} under BGP for more information.
