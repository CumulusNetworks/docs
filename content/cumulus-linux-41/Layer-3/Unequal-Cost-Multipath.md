...
- Leaf01 has two ECMP paths to 192.168.10.1/32 (via Server01 and Server03) whereas Leaf03 has a single path to Server04.
...
- Leaf01 and Leaf02 generate a link bandwidth that is twice the size of the link bandwidth generated by Leaf03 and Leaf04 when announcing the prefix to the spines.
...
The final result is each spine with four UCMP routes:
...
In this case, the Border leafs balance traffic equally; all weights are equal to the spines. Only the spines have unequal load sharing based on the weight values.
...
The BGP link bandwidth extended community is encoded in bytes-per-second. To convert the number of ECMP paths, a reference bandwidth of 1Mbps is used. For example, if there are 4 ECMP paths to an anycast IP, the encoded bandwidth in the extended community is 524,288. The actual value does not matter as long as all routers originating the link bandwidth are converting the number of ECMP paths in the same way.
...
Change this setting per BGP instance for both IPv4 and IPv6 unicast routes in the BGP instance. For EVPN, you set the options on the tenant VRF.

<details>

<summary>NCLU commands </summary>

Run the `net add bestpath bandwidth ignore|skip-missing|default-weight-for-missing` command.
...
The following commands set link bandwidth processing to skip paths without link bandwidth and do UCMP among the other paths:
...
Run the `net del bgp neighbor <neighbor> send-community extended` command:
