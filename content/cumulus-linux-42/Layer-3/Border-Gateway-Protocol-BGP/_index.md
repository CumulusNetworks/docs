...
This section describes how the IPv6 next hops are set in the MP_REACH_NLRI ({{<exlink url="https://www.ietf.org/rfc/rfc2858.txt" text="multiprotocol reachable NLRI">}}) initiated by the system, which applies whether IPv6 prefixes or IPv4 prefixes are exchanged with ENHE. There are two main aspects to determine: how many IPv6 next hops are included in the MP\_REACH\_NLRI (the RFC allows either one or two next hops) and the values of the next hops. This section also describes how a received MP\_REACH\_NLRI is handled to process IPv6 next hops.

- When peering to a global IPv6 address or link-local IPv6 address, sending one or two next hops is determined as follows:
    ...
    - If not reflecting the route, two next hops are sent if explicitly configured (`nexthop-local unchanged`) or the peer is directly connected (either peering is on the link-local address or the global IPv4 or IPv6 address is *directly connected*) and the route is either a local or self-originated route, or the peer is an eBGP peer.
    - In all other cases, only one next hop is sent, unless an outbound route map adds another next hop. `route-map` can impose two next hops, where Cumulus Linux only sends one next hop (by specifying `set ipv6 nexthop link-local`).
