...
Like other systems that use OpenAPI, the NVUE OAS schema defines the endpoints (paths) exposed as RESTful APIs. These REST APIs enable you to perform various create, retrieve, update, delete, and eXecute (CRUDX) operations. The OAS schema describes the API inputs and outputs (data models).

You can use the NVUE object model in these two ways:

- Through the NVUE REST API, where you run the GET/PATCH/DELETE/ REST APIs on the NVUE object model endpoints to configure, monitor, and manage the switch. OAS is mature and has a user community; you can use several popular tools and libraries to create client-side bindings to use the NVUE REST API.
