...
{{< /tab >}}
{{< tab "Try It " >}}
    {{< simulation name="Try It CL512 - PIM" showNodes="leaf01,leaf02,spine01,server01,server02" >}}
...
- When switch is the FHR, Cumulus Linux does not install the mroute in hardware until it receives a `Register-Stop` from the RP. Without a hardware entry, packet forwarding does not occur. In a virtual environment such as NVIDIA Air, there is no hardware component so multicast routing occurs without the need for the `Register-Stop`.
