...
The following example configures Cumulus Linux to send log files to the remote syslog server with the 192.168.0.254 IP address on port 514 using UDP.

```
cumulus@switch:~$ nv set system syslog server 192.168.0.254 port 514
cumulus@switch:~$ nv set system syslog server 192.168.0.254 protocol udp
...
## Show syslog Configuration Settings
