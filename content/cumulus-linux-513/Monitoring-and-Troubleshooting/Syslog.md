...
The following example defines the selector called SELECTOR2 and filters logs generated by `switchd` with severity level debug but excludes logs that match the regular expression `issu_start=false.+$`:
