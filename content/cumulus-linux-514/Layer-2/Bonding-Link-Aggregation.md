...
{{%notice note%}}
In NVUE, if you create the bond interface with a name that starts with `bond`, NVUE automatically sets the interface type to `bond`. If you create a bond interface with a name that does *not* start with `bond`, you must set the interface type to `bond` with the `nv set interface <interface-id> type bond` command.
{{%/notice%}}
