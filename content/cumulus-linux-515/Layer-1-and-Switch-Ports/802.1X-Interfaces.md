...
Dynamic IPv6 multi-tenancy integrates authentication, address assignment, access control, and routing to provide tenant-specific IPv6 connectivity.

With dynamic IPv6 multi-tenancy, the switch dynamically generates an IPv6 address with a global /64 prefix and configures the ports for <span class="a-tooltip">[SLAAC](## "Stateless Address Autoconfiguration")</span>.

Use this feature to isolate tenant traffic in a multi tenant environment in layer 3 IPv6 and advertise summary routes for the dynamically generated IPv6 addresses.
...
- Create an IPv6 profile and define the various parts of the IPv6 address prefix:
  ...
  - `isolation-mask` is an optional flag to specify if this row is a criterion for tenant isolation. Setting this flag causes the ACL to be programmed. If multiple rows have this flags set, then they are all combined.
...
- Associate the IPv6 profile to an interface.
- Specify the port ID for the IPv6 address (optional). You can set the port ID to auto (the default setting) to assign the port ID from the system assigned MAC address for the port.

```
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE route-tag 11 
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE property PREFIX  
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE property PREFIX offset 52
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE property PREFIX length 8  
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE property PREFIX value fdc0d000  
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE property PREFIX isolation-mask enabled
cumulus@switch:~$ nv set system dot1x ipv6-profile MYPROFILE property PREFIX summarize-out enabled
cumulus@switch:~$ nv set interface swp1 dot1x ipv6-profile MYPROFILE
cumulus@switch:~$ nv set interface swp1 dot1x port-id auto
cumulus@switch:~$ nv config apply
```
