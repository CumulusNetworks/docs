...
When a supplicant sends an EAPOL message, the `hostapd` service authenticates the supplicant with the RADIUS server and the authentication message from the RADIUS server triggers the following steps.

The switch:
1. Starts layer 3 processing.
2. Handles the <span class="a-tooltip">[VSA](## "Vendor Specific Attribute")</span> in the Radius message.
3. Generates the IPv6 address for the interface assigns the address to the port.
4. Handles multi-tenant isolation.
5. Generates route summary and updates FRR.
...
- Create an IPv6 profile and define the various parts of the IPv6 address prefix:
  - `offset` is the starting offset of the prefix substring in a row.
  - `length` is the length of the prefix substring in a row.
  - `value` is a string, integer value, enum value of the VSA, or port ID.
  ...
  - `route-tag` is the route summary tag value.
...
```
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE route-map-tag 911 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PREFIX  
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PREFIX offset 0   
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PREFIX length 32 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PREFIX value fdc0d000 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PREFIX isolation-mask enabled 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TOR_ID offset 32  
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TOR_ID length 8 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TOR_ID value 42 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TENANT-ID offset 40  
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TENANT-ID length 12 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TENANT-ID value vsa 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property TENANT-ID isolation-mask enabled 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PORT offset 52 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PORT length 8  
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PORT value port-id 
cumulus@switch:~$ nv set system dot1x ipv6-profile PORSCHE property PORT summarize-out enabled
cumulus@switch:~$ nv set interface swp9,swp10 dot1x ipv6-profile PORSCHE 
...
Create the `/etc/hostapd/ipv6_profiles.conf` file and define the various parts of the IPv6 address prefix for each tenant:
...
---------  -----------------  ----------  -------------------   -------  ------------ 
swp1       00:02:00:00:00:05  MYPROFILE   FDC0:D000:0017:B005   005      7B
swp2       00:02:00:00:00:06  MYPROFILE2  FDC0:D000:0017:C006   006      7C
swp3       00:04:00:01:07:D0  MYPROFILE3  FDC0:D000:0018:A7D0   7D0      8A
```
