...
- **Required**: Provide the 802.1X RADIUS shared secret. The shared secret can be a maximum of 256 characters long.
...
- `optional` to use the VRF if it is present in the Radius authentication message.
- `required` to enforce VRF assignment on layer 3 interfaces. If the RADIUS authentication response does not contain a VRF, authentication fails.
- `disabled` to disable dynamic VRF assignment. This is the default setting.
...
- Set the route tag for the profile.
...
{{%notice note%}}
For an IPv6 profile property with a literal value (a decimal or hexadecimal constant and typically the prefix property at offset 0), the length must be 32 bits or fewer. NVUE accepts a length of up to 64 bits but values wider than 32 bits are not applied correctly; the upper length (32 bits of the assembled IPv6 address) are replaced with all zeros or all ones. Depending on the result, the station either authorizes with an incorrect prefix or, if the address falls into the multicast range ff00::/8, the station rejects the address and the station fails to authorize. This restriction does not apply to properties whose value is `vsa-id` or `port-id`, which Cumulus Linux substitutes at runtime.
{{%/notice%}}
...
### Preserve Dynamically Assigned IPv6 Addresses

In an 802.1x IPv6 profile configuration, all system states reset to default when a supplicant deauthenticates, regardless of the reason because 802.1x deletes the dynamically assigned IPv6 address, removing the connected route. For certain features, such as {{<link url="Optional-BGP-Configuration/#bgp-conditional-disaggregation" text="BGP conditional disaggregation">}}, the route must remain present but invalid when the egress interface goes down, so it can be advertised.

To preserve the IPv6 addresses when the switch reboots or a link flaps, you must enable the `preserve-on-link-down` option:
...
To show the dynamic IPv6 multi-tenancy profiles configured on the switch, run the `nv show system dot1x ipv6-profile` command. To show information about a specific dynamic IPv6 multi-tenancy profile, run the `nv show system dot1x ipv6-profile <profile-id>` command.
...
802.1X reauthentication ensures that the switch revalidates connected clients for authorization and security after a port flap or switch reboot. When a port goes down and comes back up, reauthentication:
...
802.1X reauthentication is disabled by default. To enable 802.1x reauthentication, run the `nv set system dot1x tx-identity-request state enabled` command:
...
To disable 802.1x reauthentication, run the `nv set system dot1x tx-identity-request state disabled` command.

You can set the following options:
- The `identity-request delay`, which is the number of seconds to wait after a link-up event before sending the first EAP Identity Request. You can set a value between 1 and 60. The default value is 1 second.
- The `identity-request interval`, which is the frequency in seconds at which EAP Identity Requests are sent until the switch detects a supplicant. You can set a value between 1 and 3600. The default value is 30 seconds.

The following example sets the identity-request delay to 10 seconds and the identity-request interval to 10 seconds:
...
To show 802.1x reauthentication configuration, run the `nv show system dot1x tx-identity-request` command.

```
cumulus@switch:~$ nv show system dot1x tx-identity-request
               operational  applied
-------------- ----------   --------
...
Dynamic VRF assignment is useful when you need to dynamically place users or devices into different VRFs based on authentication, policies, or network conditions.

When you enable dynamic VRF assignment, the switch blocks all traffic except the supplicant EAPOL messages. When the supplicant is authenticated with RADIUS, RADIUS includes a VSA that contains the VRF. The `hostapd` service parses this VSA, does some validation and assigns the 802.1x interface to the VRF.

To use dynamic VRF assignment on the switch, you need to configure RADIUS with the NVIDIA Vendor Specific Attribute (VSA). The VSA has the following structure:

| Field | Size in bytes | Description |
| ----- | ------------- | ------------- |
| Type | 1 | The VSA identifier in RADIUS; for example, 26. |
| Length | 1 | The total length; for example, 16. |
| Vendor ID | 4 | Nvidia PEN 5703; for example, 00 00 16 47. |
| Vendor Type | 1 | 2 bytes (Type+Length) + VSA payload length; for example, 02. |
| Vendor Length | 1 | The vendor data length. The maximum length is 15. |
| Vendor Data | variable | The VRF; for example, `“VRF-BLUE”`. |

{{%notice note%}}
- You can enable dynamic VRF assignment on layer 3 switch ports only.
- When you enable and disable dynamic VRF assignment, the `hostapd` service restarts causing all the sessions to go down, which requires reauthentication.
{{%/notice%}}

Dynamic VRF assignment uses the following options:
- `required` enforces dynamic VRF assignment on layer 3 interfaces. If the RADIUS authentication response does not contain the VRF, authentication fails. 
- `optional` uses the VRF if it is present.
- `disabled` disables dynamic VRF assignment. Normal session authentication occurs. This is the default setting.

{{< tabs "TabID920 ">}}
{{< tab "NVUE Commands ">}}

Run the `nv set system dot1x dynamic-vrf` command. The following example sets dynamic VRF assignment to `required`.

```
cumulus@switch:~$ nv set system dot1x dynamic-vrf required
cumulus@switch:~$ nv config apply
```

To disable dynamic VRF assignment, run the `nv unset system dot1x dynamic-vrf` command or the `nv set system dot1x dynamic-vrf disabled` command.

{{< /tab >}}
{{< tab "Linux Commands ">}}

Edit the `/etc/hostapd.conf` file to set the `dynamic_vrf` option, then restart the `hostapd` service.

```
cumulus@switch:~$ sudo nano /etc/hostapd.conf
...
dynamic_vrf=required
...
```
...
To show the dynamic VRF assignment configuration, run the `nv show system dot1x` command.
...
reauth-timeout-ignore                   no 
dynamic-vrf                             required     
...
radius                                          
  [server]                                      
[ipv6-profile]                                  
dynamic-ipv6-multi-tenant                       
  state                    disabled     disabled
...
```

To show the VRF for an interface, run the `nv show interface dot1x-summary` command or the `nv show interface <interface-id>` command.

```
cumulus@switch:~$ nv show interface dot1x-summary 
Interface Mac-Address       Status     Auth-Type Username Vlan VRF        Session-id 
--------- ----------------- ---------  --------- -------- ---- ---------- ---------------- 
swp1      00:02:00:00:00:05 AUTHORIZED MD5       host01         BLUE      D5E6EB7A8B7FB69F
```
-->
## Troubleshooting
