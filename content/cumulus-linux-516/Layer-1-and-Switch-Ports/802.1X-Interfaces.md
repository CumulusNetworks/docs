...
{{%notice note%}}
For an IPv6 profile property with a literal value (a decimal or hexadecimal constant, typically the prefix property at offset 0), the length must be 32 bits or fewer. NVUE accepts a length of up to 64 bits, but does not apply values wider than 32 bits correctly: the upper `length` minus 32 bits of the assembled IPv6 address become all zeros or all ones. The supplicant then either authorizes with an incorrect prefix or, if the assembled address falls into the multicast range ff00::/8, fails to authorize. This restriction does not apply to properties whose value is `vsa-id` or `port-id`, which Cumulus Linux substitutes at runtime.
{{%/notice%}}
