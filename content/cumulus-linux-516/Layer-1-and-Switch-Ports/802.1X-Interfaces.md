...
- `disabled` disables dynamic VRF assignment.
...
Edit the `/etc/hostapd.conf` file to add the ?? option, then restart the `hostapd` service.
...
When you enable and disable dynamic VRF assignment, `hostapd` restarts causing all the sessions to go down, which requires reauthentication.
