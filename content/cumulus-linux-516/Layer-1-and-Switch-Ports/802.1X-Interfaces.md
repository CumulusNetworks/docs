...
In an 802.1x IPv6 profile configuration, all system states reset to default when a supplicant deauthenticates, regardless of the reason because 802.1x deletes the dynamically assigned IPv6 address, removing the connected route. For certain features, such as {{<link url="Optional-BGP-Configuration/#bgp-conditional-disaggregation" text="BGP conditional disaggregation">}}, the route must remain present but invalid, which occurs when the egress interface goes down, so it can be advertised.
...
| ----- | ------------- | ----- | ----------- |
| Type | 1 | 26 | VSA identifier in RADIUS |
| Length | 1 | | Total length |
| Vendor ID | 4 | 00 00 16 47 | Nvidia PEN 5703 |
| Vendor Type | 1 | 02 | 2 bytes (Type+Length) + VSA payload length |
...
To enable dynamic VRF assignment, run the `nv set system dot1x dynamic-vrf` command. You can set the following options:
...
- `disabled` disables dynamic VRF assignment. Normal session authentication occurs.
...
Set the `dynamic_vrf` option to:
- `2` to enforce dynamic VRF assignment on layer 3 interfaces. If the RADIUS authentication response does not contain the VRF, authentication fails. 
- `1`to use the VRF if it is present.
- `0` to disable dynamic VRF assignment. Normal session authentication occurs.
