...
- 802.1X reauthentication on an interface. You can enable or disable reauthentication on specific interfaces instead of globally, which enables 802.1X reauthentication on all 802.1x interfaces.
...
The following example sets the maximum number of retries to 60 and enables 802.1X reauthentication on swp1:

```
cumulus@switch:~$ nv set system dot1x tx-identity-request max-retries 50
cumulus@switch:~$ nv set interface swp1 dot1x tx-identity-request enabled
cumulus@switch:~$ nv config apply
```
