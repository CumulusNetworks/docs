...
- The `identity-request interval`, which is the frequency in seconds at which EAP Identity requests are sent until the switch detects a supplicant, or you reach the maximum retries (where the maximum retries setting is not unlimited but a finite value). You can set a value between 1 and 3600. The default value is 30 seconds.
- The `max-retries` (maximum number of multicast EAPOL identity request retries). By default, the switch sends retries indefinitely at 30-second intervals until at least one supplicant responds to an EAPOL identity request with an EAP identity response. You can set a value between 0 and 65535.
- 802.1X reauthentication on an interface. You can enable or disable reauthentication on specific interfaces instead of globally. Enabling 802.1X reauthentication globally enables 802.1X reauthentication on all 802.1x interfaces.
...
The following example sets the maximum number of retries to 50 and disables 802.1X reauthentication on swp1:

```
cumulus@switch:~$ nv set system dot1x tx-identity-request max-retries 50
cumulus@switch:~$ nv set interface swp1 dot1x tx-identity-request disabled
cumulus@switch:~$ nv config apply
```
...
To show the unsolicited request/identity frame values for all the 802.1x interfaces on the switch, run the `nv show interface dot1x-counters` command:
...
---------  -----------------  --------  --------  ------  -------  ------  --------  ---------  ----------  ----------  ---------  ----------------- 
swp1    00:02:00:00:00:01  3         3         2       3        0       0         1          1           0           0          Reauth Timeout: 0 

Interface  Unsolicited Request/Identity TX 
---------  ----------------- 
swp1       67 
swp2       21
```

To show the number of unsolicited identity request frames for an interface, run the `nv show interface <interface> dot1x` command
...
ipv6-profile                                                          
port-id                              0xcae7                    auto   
tx-identity-request                  disabled                  enabled
unsolicited-identity-request-frames  21                                
...
To show if Cumulus Linux blocks LLDP packets on unauthorized ports, run the `nv show system dot1x` command:
...
-------------------------  -----------  --------- 
reauthentication-interval  0            0 
reauth-timeout-ignore      no           no 
dynamic-vrf                required     required 
dynamic-vlan               disabled     disabled 
max-stations               6            6 
logging-level                           debug 
radius 
  client-src-ip            101.1.1.1    101.1.1.1 
  [server]                 101.1.1.2    101.1.1.2 
  nas-ip-address           3.3.3.3      3.3.3.3 
  nas-identifier           test123!     test123! 
[ipv6-profile] 
