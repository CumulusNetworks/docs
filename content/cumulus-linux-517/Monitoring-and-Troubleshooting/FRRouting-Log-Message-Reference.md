...
<!--
{{%notice note%}}
An asterisk (*) in the `Message Text` column of the table below indicates that log message is new for Cumulus Linux 5.18.
{{%/notice%}}
-->
...
| BGP | HIGH | 33554516 | BGP route attribute is malformed or invalid | BGP received or processed a route with malformed attributes (prefix, RD, ESI, MAC, gateway IP, layer 2 VPN prefix data, address family mismatches, route-map or label-index mismatches) or referenced route not found. | Check route definitions, and EVPN and layer 2 VPN configuration. Verify peer configuration for attribute consistency, address family matching, and static route configuration alignment. Ensure all referenced routes exist and route data format is correct.|
| BGP | HIGH | 33554526 | BGP TTL security configuration failed | BGP failed to configure TTL security parameters (min or max TTL) on peer connection. | Ensure both peers support TTL security and verify that the kernel supports TTL security socket options. Check the peer TTL security configuration for compatibility.|
| BGP | HIGH | 33554520 | BGP failed to decode message from Zebra | BGP received a malformed or incompatible message from the Zebra daemon. | Ensure BGP and Zebra are running the same FRR version. Restart both daemons if needed.|
| BGP | HIGH | 33554519 | BGP VRF not found | The BGP requested VRF does not exist or cannot be found. | Create the required VRF or correct the configured VRF name.|
...
| OSPF | HIGH |  134217754 | OSPF LDP synchronization down | OSPF-LDP sync detected that the LDP session went down. | Check LDP sessions and restart LDP if necessary. Verify OSPF-LDP sync configuration. |
...
| Zebra | HIGH | 4043309106 | FPM message encoding failed | The Zebra FPM module failed to encode netlink messages for the forwarding plane (route multipath, MAC FDB, nexthop, LSP, or prefix encoding).| Check data consistency (routes must have valid nexthops, MACs must have valid interfaces). Verify the kernel netlink feature support and review system logs.|
| Zebra | HIGH | 4043309107 | Zebra received extended error from netlink | The Kernel returned a netlink extended error message. | Review the kernel logs for the reason and check netlink extended error information. |
| Zebra | HIGH | 4043309112 | Zebra received invalid prefix length from kernel | The Kernel reported an invalid prefix length in a route change notification. The Kernel sent invalid route data to FRR. | Check system logs for hardware errors. |
| Zebra | HIGH | 4043309125 | Graceful restart disconnect handling failed | Graceful restart is enabled but the disconnect event handler failed. | Check for duplicate routing protocol instances and verify that the graceful restart stale removal timers are  configured properly. |
| Zebra | HIGH | 4043309126 | SRv6 resource allocation failed | SRv6 resource allocation failed (locator chunk or SID function allocation). | Check SRv6 configuration for available address space, resource limits, and potential conflicts. Verify locator chunk sizes and SID function assignments. |
| Zebra | HIGH | 134217754 | L2 MAC hash table creation failed | Failed to create a MAC hash table for bridge and VLAN. | Check system memory and bridge configuration. Verify VLAN settings. |
<!-- vale on -->
