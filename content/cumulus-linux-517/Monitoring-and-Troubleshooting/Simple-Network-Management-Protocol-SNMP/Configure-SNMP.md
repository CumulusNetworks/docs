...
Cumulus Linux can send proactive alerts as SNMPv3 traps to configured trap destinations (and as {{<link url="Syslog/#enable-security-alerts" text="syslog messages ">}}) when audit processing failures occur, such as when the audit daemon crashes or disk space runs low on the audit partition.
...
Security alerts are disabled by default. To enable global security alerts:
...
You can enable or disable individual alert categories. Currently, `audit-failure` is the only supported category and is enabled by default.
