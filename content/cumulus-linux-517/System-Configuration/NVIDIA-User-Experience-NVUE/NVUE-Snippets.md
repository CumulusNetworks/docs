...
- NVUE does not validate the snippet text but does ensure that the destination conforms to NVUE policy during apply. NVUE policies prevent writing to the `/bin`, `/sbin`, `/usr/bin`, `/usr/sbin`, `/lib`, `/lib64`, `/boot`, `/root`, `/sys`, `/proc`, `/dev`, `/var/lib/dpkg`, and `/var/lib/rpm` directories.
