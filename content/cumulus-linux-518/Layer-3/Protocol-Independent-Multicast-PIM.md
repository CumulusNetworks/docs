...
{{%notice note%}}
NVUE does *not* validate the prefix list that the IP multicast boundary OIL references​​​​​​. Make sure the prefix list exists before you run the `nv config apply` command.
{{%/notice%}}
...
- When the switch is the FHR, Cumulus Linux does not install the mroute in hardware until it receives a `Register-Stop` from the RP. Without a hardware entry, packet forwarding does not occur. In a virtual environment such as NVIDIA DSX Air, there is no hardware component so multicast routing occurs without the need for the `Register-Stop`.
