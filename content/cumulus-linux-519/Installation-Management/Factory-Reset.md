...
- If you run factory reset after a package update or install, the switch might get into an unusable state if the new installed packages create configuration files under the `/etc` directory. Running factory reset restores the `/etc` directory from the initial configuration backup archive and the changes in the `/etc` directory are lost. To recover from the unusable state, uninstall the new package, then run factory reset or reinstall Cumulus Linux with ONIE.
