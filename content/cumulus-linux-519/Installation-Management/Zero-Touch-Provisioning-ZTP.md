...
4. The ZTP process requests the contents of the script from the URL, sending additional {{<link url="#inspect-http-headers" text="HTTP headers">}} containing details about the switch.
   ...
   If the ZTP process fails to retrieve the script from the URL after three attempts, the switch renews the DHCP lease and ZTP tries again. This process enables the device to obtain an updated DHCP option 239 URL without requiring a reboot or waiting for lease expiry, allowing provisioning to converge shortly after correcting the DHCP server.
...
- Send the ONIE installer an empty ZTP script with the `nv action install system image onie <image> ztp <URL_to_empty_ztp_script>` or `onie-install -z <URL_to_empty_ztp_script>` command. The following example shows an empty ZTP script:
...
- You can use the `nv action boot-next system image onie install` or the `onie-select -i` command to reprovision the switch and install a network operating system again using ONIE.
