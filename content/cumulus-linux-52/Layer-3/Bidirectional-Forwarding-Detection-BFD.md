...
Use [FRR](## "FRRouting") to register multihop peers with {{<link url="Prescriptive-Topology-Manager-PTM" text="PTM">}} and BFD, and monitor the connectivity to the remote [BGP](## "Border Gateway Protocol") multihop peer. FRR can dynamically register and unregister both IPv4 and IPv6 peers with BFD when the BFD-enabled peer connectivity starts or stops. Also, you can configure BFD parameters for each BGP or [OSPF](## "Open Shortest Path First") peer.
