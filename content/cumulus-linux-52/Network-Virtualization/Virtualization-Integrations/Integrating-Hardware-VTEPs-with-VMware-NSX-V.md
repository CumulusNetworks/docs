...
- The IP address of the management interface on the switch (`192.168.100.157` in the example command below). This interface is for control traffic.
...
2. In the `/usr/share/openvswitch/scripts/ovs-ctl-vtep` file, make sure the lines containing **private-key**, **certificate**, and **bootstrap-ca-cert** point to the correct files; Cumulus Linux obtains **bootstrap-ca-cert** dynamically the first time the switch talks to the controller:
    ...
    If files move or regenerate, restart the OVSDB server and VTEPd:
...
1. In NSX Manager, add a new HW VTEP gateway. Select the **Network & Security** icon, **Service Definitions** category, then the **Hardware Devices** tab. Under **Hardware Devices**, select **+**. The Create Add Hardware Devices window opens.
...
5. Select **OK** to save the gateway.  

    {{< img src = "/images/cumulus-linux/virtualization-integrations-nsxv-mgr-svc.png" >}}

After communication establishes between the switch and the controller, a `controller.cacert` file downloads onto the switch.
...
1. In NSX Manager, select the **Logical Network Preparation** tab in the **Installation** category, then select the **Segment ID** tab.

2. Select **Edit** and add the segment IDs (VNIDs) you want to use. The example configures VNIs 5000-5999.
...
3. Select **OK** to save and provision the segment IDs.
4. Select the **Transport Zones** tab, choose the name of the transport zone.  
...
6. Select **OK** to save the new transport zone.
...
1. In NSX Manager, select the **Logical Switches** category. Select **+** to add a logical switch instance.  
...
1. Select the **Service Definitions** category, then select the **Hardware Devices** tab. Next to the **Replication Cluster** field, select **Edit**.

    {{< img src = "/images/cumulus-linux/virtualization-integrations-nsxv-replication-cluster.png" >}}

2. Hypervisors connected to the NSX controller for replication appear in the **Available Objects** list. Select the required service nodes, then select the green arrow to move them to the **Selected Objects** list.

    {{< img src = "/images/cumulus-linux/virtualization-integrations-nsxv-replication-cluster-save.png" >}}

3. Select **OK** to save the replication node configuration.
...
1. In NSX Manager, add a new logical switch port. Select the **Logical Switches** category. Under **Actions**, select **Manage Hardware Bindings**. The Manage Hardware Binding wizard appears.

    {{< img src = "/images/cumulus-linux/virtualization-integrations-nsxv-hw-bindings.png" >}}

2. Select **+** to add a logical port to the logical switch.  
...
5. Select **OK** to save the logical switch port. Repeat this procedure for each logical switch port you want to define.  
