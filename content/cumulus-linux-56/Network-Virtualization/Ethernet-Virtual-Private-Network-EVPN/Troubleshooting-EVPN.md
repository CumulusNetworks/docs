...
## Reset VNI Statistics

You can reset statistics for all VNIs, a specific VNI, a VNI host IP address, or a VNI MAC address.

{{< tabs "TabID820 ">}}
{{< tab "NVUE Commands ">}}

To reset statistics for all VNIs:

```
cumulus@switch:~$ nv action clear evpn vni
Running handle_clear_evpn_vnis
Action succeeded
```

To reset statistics for a specific VNI:

```
cumulus@switch:~$ nv action clear evpn vni 10
Running handle_clear_evpn_vni 10
Action succeeded
```

To reset statistics for a VNI host IP address:

```
cumulus@switch:~$ nv action clear evpn vni 10 host 10.10.10.1
Running handle_clear_evpn_vni_host 10
Action succeeded
```

To reset statistics for a VNI MAC address:

```
cumulus@switch:~$ nv action clear evpn vni 10 mac 00:e0:ec:20:12:62
Running handle_clear_evpn_vni_mac 10
Action succeeded
```
...
To rest statistics for all VNIs:
...
cumulus@switch:~$ sudo vtysh
...
switch# clear evpn dup-addr vni 10
switch# write memory
switch# exit
```
...
cumulus@switch:~$ sudo vtysh
...
switch# clear evpn dup-addr vni 10 ip 10.10.10.1
switch# write memory
switch# exit
```
