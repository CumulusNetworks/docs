...
3. After upgrade is complete, restore the configuration. Copy the `backup.config` file to the switch, then run the `source backup.config` command to run all the commands in the file.
   ...
   {{%notice note%}}
   If the backup configuration contains any obfuscated passwords, you must reset the passwords after running the `source backup.config` command to prevent authentication from failing.
   {{%/notice%}}
