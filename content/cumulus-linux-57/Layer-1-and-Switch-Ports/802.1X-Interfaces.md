...
If a non-authorized supplicant tries to communicate with the switch, you can route traffic from that device to a different VLAN and associate that VLAN with one of the switch ports to which the supplicant is attached. Cumulus Linux assigns the auth-fail VLAN by manipulating the PVID of the interface.
...
If the authentication for swp1 fails, the interface is moved to the auth-fail VLAN.
