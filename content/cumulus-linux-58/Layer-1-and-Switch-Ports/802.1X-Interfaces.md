...
Cumulus Linux provides the following host modes:
...
- Multi-host mode, where the interface remains closed for all traffic until RADIUS authorizes the first supplicant. After authorization, any host can send or receive traffic through the 802.1X interface as long as supplicant remains authorized.

To configure the 802.1X mode:

{{< tabs "TabID474 ">}}
{{< tab "NVUE Commands ">}}

The following example configures multi-host mode on the 802.1X interface swp1:
...
The following example configures host mode back to the default (multi-host-authenticated) on the 802.1X interface swp1:
...
To go back to the default host mode configuration, you can also run the `nv unset interface swp1 dot1x host-mode multi-host` command.
...
acct_server_shared_secret=mysecret 
multihost_interfaces=swp1,swp2
```
