...
By default, multi-host-authenticated’ mode, where each supplicant needs to be authorized by radius to be able to send traffic via switchport with dot1x enabled.

The new host-mode introduced in CL 5.8.0 is ‘multi-host’. In multi-host mode, port remains closed for all traffic until first supplicant is authorized. Once authorized, any host should be able to send/receive traffic via switchport as long as supplicant stays authorized.

nv set interface <interface> dot1x host-mode multi-host

nv set interface <interface> dot1x host-mode multi-host-authenticated
