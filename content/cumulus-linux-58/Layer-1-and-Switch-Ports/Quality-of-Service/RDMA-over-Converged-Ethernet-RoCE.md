...
- RoCE configuration is different depending on the switch ASIC (Spectrum 1, Spectrum-2, or Spectrum-3).
{{%/notice%}}
...
To enable RoCE with PFC and ECN (lossless):
...
{{% notice note %}}
NVUE defaults to `roce mode lossless`. The `nv set qos roce` and `nv set qos roce mode lossless` commands are equivalent.

If you enable `mode lossy`, configuring `nv set qos roce` without a `mode` does not change the RoCE mode. To change to lossless, you must configure lossless mode with the `nv set qos roce mode lossless` command.
{{% /notice %}}
