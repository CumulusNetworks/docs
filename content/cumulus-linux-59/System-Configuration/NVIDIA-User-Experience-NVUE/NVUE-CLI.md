...
- The default startup configuration includes the local user account for the `cumulus` user. You can not use the `nv config replace` command to install a configuration without the `cumulus` account configured while logged into the switch as the `cumulus` user. To merge in configuration changes or to restore a previously backed up `startup.yaml` file, use the `nv config patch` command instead.
{{%/notice%}}
