...
weight: 45
aliases:
 - /display/CL40/Upgrading-Cumulus-Linux
...
## <span id="src-8366370_UpgradingCumulusLinux-before-upgrade" class="confluence-anchor-link"></span><span id="src-8366370_UpgradingCumulusLinux-FilesToNeverMigrate" class="confluence-anchor-link"></span><span>Before You Upgrade Cumulus Linux</span>
...
| --------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `/etc/network/`             | Network configuration files, most notably `/etc/network/interfaces` and `/etc/network/interfaces.d/` | [Switch Port Attributes](/version/cumulus-linux-40/Layer-1-and-Switch-Ports/Interface-Configuration-and-Management/Switch-Port-Attributes)           | N/A                                                                                                                          |
...
| `/etc/frr/`                 | Routing application (responsible for BGP and OSPF)                                                   | [FRRouting Overview](/version/cumulus-linux-40/Layer-3/FRRouting-Overview/)                                                                          | N/A                                                                                                                          |
| `/etc/hostname`             | Configuration file for the hostname of the switch                                                    | [Quick Start Guide](Quick-Start-Guide.html#src-8366263_QuickStartGuide-ConfiguringtheHostnameandTimeZone)                                            | [wiki.debian.org/HowTo/ChangeHostname](https://wiki.debian.org/HowTo/ChangeHostname)                                         |
| `/etc/cumulus/acl/*`        | Netfilter configuration                                                                              | [Netfilter - ACLs](/version/cumulus-linux-40/System-Configuration/Netfilter---ACLs/)                                                                 | N/A                                                                                                                          |
| `/etc/cumulus/ports.conf`   | Breakout cable configuration file                                                                    | [Switch Port Attributes](Switch-Port-Attributes.html#src-8366750_SwitchPortAttributes-ConfiguringBreakoutPorts)                                      | N/A; please read the guide on breakout cables                                                                                |
| `/etc/cumulus/switchd.conf` | Switchd configuration                                                                                | [Configuring switchd](/version/cumulus-linux-40/System-Configuration/Configuring-switchd)                                                            | N/A; please read the guide on switchd configuration                                                                          |
...
<td><p><a href="/version/cumulus-linux-40/Layer-2/Link-Layer-Discovery-Protocol/">Link Layer Discovery Protocol</a></p></td>
...
<td><p><a href="/version/cumulus-linux-40/System-Configuration/Authentication-Authorization-and-Accounting/TACACS-Plus">TACACS Plus</a></p></td>
<td><p>N/A</p></td>
</tr>
<tr class="even">
<td><p><code>/etc/ssh/</code></p></td>
<td><p>SSH configuration files</p></td>
<td><p><a href="/version/cumulus-linux-40/System-Configuration/Authentication-Authorization-and-Accounting/SSH-for-Remote-Access">SSH for Remote Access</a></p></td>
...
<td><p><a href="/version/cumulus-linux-40/System-Configuration/Authentication-Authorization-and-Accounting/Using-sudo-to-Delegate-Privileges">Using sudo to Delegate Privileges</a></p></td>
<td></td>
</tr>
</tbody>
</table>
...
<summary>Never Migrate these Files between Versions or Switches
</summary>
...
<span id="src-8366370_UpgradingCumulusLinux-pkg_upgrade_notes"></span>If
you are using certain forms of [network
virtualization](/version/cumulus-linux-40/Network-Virtualization/),
including [VMware
NSX-V](/version/cumulus-linux-40/Network-Virtualization/Virtualization-Integrations/Integrating-Hardware-VTEPs-with-VMware-NSX-V)
or [Midokura
...
to back up this file and the database file </span> `conf.db`
<span style="color: #36424a;"> before upgrading. </span>

## <span>Upgrade Cumulus Linux</span>

To upgrade to Cumulus Linux 4.0.0, you must install a disk image of the
new release using ONIE. You *cannot* upgrade packages with the `apt-get
upgrade` command.
...
Pair](#src-8366370_UpgradingCumulusLinux-mlag) below to ensure a smooth
upgrade.
...
Be aware of the following when installing the disk image:
  ...
  - Moving a configuration file might cause issues:
      ...
      - Identifying all the locations of configuration data is not
        ...
        Linux](#src-8366370_UpgradingCumulusLinux-before-upgrade) above.
...
## <span id="src-8366370_UpgradingCumulusLinux-mlag" class="confluence-anchor-link"></span><span>Upgrade Switches in an MLAG Pair</span>
...
3.  Shut down the peerlink:
    
        cumulus@switch:~$ sudo ip link set peerlink down

4.  Run the `onie-install -a -i` `  <image-location> ` command to boot
...
7.  Verify core uplinks and peerlinks are UP:
...
12. Remove the priority 2048 and restore the priority back to 32768 on
...
any third party packages or any Cumulus Linux add-on packages.
...
## <span>Related Information</span>
  ...
  - [Zero Touch Provisioning -
    ZTP](/version/cumulus-linux-40/Installation-Management/Zero-Touch-Provisioning---ZTP)
