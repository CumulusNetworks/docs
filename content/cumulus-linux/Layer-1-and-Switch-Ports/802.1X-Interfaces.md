  ...
  - Cumulus Linux 3.7.4 and later includes support for Multi Domain
    ...
    authorization:
      ...
      - MDA is enabled by default; however, you need to assign a tagged
        ...
        VLAN-aware Bridge](#configure-802-1x-interfaces-for-a-vlan-aware-bridge)).
      ...
      - The MAB timer is not required (see [Configure MAC Authentication
        Bypass](/cumulus-linux/Layer-1-and-Switch-Ports/802.1X-Interfaces/#configure-mac-authentication-bypass)).
...
Server](#configure-the-radius-server), above for
details.
...
There is a chance that the PC attempts 802.1X authorization (and fails) through the bridged connection in the back of the phone before the phone completes MAB authorization.
