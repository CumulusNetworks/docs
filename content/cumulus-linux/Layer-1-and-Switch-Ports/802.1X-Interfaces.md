...
[NCLU](../../System-Configuration/Network-Command-Line-Utility-NCLU/) handles all the 802.1X interface configuration, updating `hostapd` and other components so you do not have to manually modify configuration files.
...
3. Enable then restart the `hostapd` service so that the configuration ersists between reboots:
