...
weight: 24953
aliases:
 - /display/CL3740/Data-Center-Host-to-ToR-Architecture
 - /pages/viewpage.action?pageId=83629916715
pageID: 83629916715
...
<tr class="odd">
<td><details>
<p><a href="/version/cumulus-linux-37740/Layer-2/Bonding---Link-Aggregation">Bond</a>/Etherchannel is not configured on host to multiple switches (bonds can still occur but only to one switch at a time), so leaf01 and leaf02 see two different MAC addresses.</p>
<p><strong>Configurations</strong></p>
<p><strongsummary>leaf01 Cconfig</strong></puration </summary>
...
<p><strongsummary>Example HUbuntu host Cconfig (Ubuntu)</strong></puration </summary>
<pre><code>auto eth1
...
<li><p>Ability to use <a href="/version/cumulus-linux-37740/Layer-2/Spanning-Tree-and-Rapid-Spanning-Tree">spanning tree</a> commands</p>
<ul>
<li><p>mstpctl-portadminedge</p></li>
<li><p><a href="Spanning-Tree-and-Rapid-Spanning-Tree.html#src-83626896412_SpanningTreeandRapidSpanningTree-bpdu">BPDU guard</a></p></li>
</ul></li>
...
<td><ul>
<li><p><a href="/version/cumulus-linux-37740/Layer-2/Virtual-Router-Redundancy---VRR-and-VRRP">VRR</a></p></li>
</ul>
<p><strong><br />
</strong></p></td>
<td><details>
...
### <span id="src-83629916715_DataCenterHosttoToRArchitecture-mlag" class="confluence-anchor-link"></span><span>MLAG</span>
...
<tr class="odd">
<td><details>
<p><a href="/version/cumulus-linux-37740/Layer-2/Multi-Chassis-Link-Aggregation---MLAG">MLAG</a> (multi-chassis link aggregation) is when both uplinks are utilized at the same time. VRR gives the ability for both spines to act as gateways simultaneously for HA (high availability) and <a href="/version/cumulus-linux-37740/Network-Virtualization/VXLAN-Active-Active-Mode">active-active mode</a> (both are being used at the same time).</p>
<p><strong>Configurations</strong></p>
<p><strongsummary>leaf01 Cconfig</strong></puration </summary>
<pre><code>auto bridge
iface bridge
...
<ul>
<li><p>Can be done with either the <a href="/version/cumulus-linux-37740/Layer-2/Ethernet-Bridging---VLANs/">traditional</a> or <a href="/version/cumulus-linux-37740/Layer-2/Ethernet-Bridging---VLANs/VLAN-aware-Bridge-Mode">VLAN-aware</a> bridge driver depending on overall STP needs</p></li>
...
<tbody>
<tr class="odd">
<td><p>Routing on the host means there is a routing application (such as <a href="/version/cumulus-linux-37740/Layer-3/FRRouting-Overview/">FRRouting</a>) either on the bare metal host (no VMs/containers) or the hypervisor (for example, Ubuntu with KVM). This is highly recommended by the Cumulus Networks Professional Services team.</p></td>
<td><p><strong>Benefits</strong></p>
<ul>
...
<li><p><a href="/version/cumulus-linux-37740/Layer-3/Configuring-FRRouting/">Configuring FRRouting</a></p></li>
</ul></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><p>Virtual router (vRouter) runs as a VM on the hypervisor/host, sends routes to the ToR using <a href="#src-8362991" class="unresolved/version/cumulus-linux-40/Layer-3/Border-Gateway-Protocol---BGP">BGP</a> or <a href="/version/cumulus-linux-37740/Layer-3/Open-Shortest-Path-First---OSPF">OSPF</a>.</p></td>
<td><p><strong><strong>Benefits</strong></strong></p>
...
<p><strong><strong>Caveats</strong></strong></p>
<ul>
<li><p><a href="/version/cumulus-linux-37740/Layer-3/Equal-Cost-Multipath-Load-Sharing---Hardware-ECMP">ECMP</a> might not work correctly (load balancing to multiple ToRs); Linux kernel in older versions is not capable of ECMP per flow (does it per packet)</p></li>
<li><p>No L2 adjacency between servers without VXLAN</p></li>
</ul></td>
</tr>
<tr class="even">
...
<p><strong>Configurations</strong></p>
<p><strongsummary>leaf01 Cconfig</strong></p>
<p><codeurations </summary>
<p>/etc/network/interfaces</code></p>
<pre><code>auto swp1
iface swp1
  address 172.16.1.1/30</code></pre>
<p><code>/etc/frr/frr.conf</code></p>
...
interface swp1
  ip ospf area 0</code></pre>
<p><strongsummary>leaf02 Cconfig</strong></purations </summary>
<p><code>/etc/network/interfaces</code></p>
<pre><code>auto swp2
iface swp2
  address 172.16.1.1/30</code></pre>
...
**LNV with MLAG: EVPN with Symmetric VXLAN Routing</span>
...
<tr class="odd">
<td><p> </p></td>
<td><p>The host runs LACP (Etherchannel/bond) to the pair of ToRs.details>
<p><a href="/version/cumulus-linux-40/Network-Virtualization/VXLAN-Routing">Symmetric VXLAN routing</a> is configured directly on the ToR, using <a href="/version/cumulus-linux-37740/Network-Virtualization/Lightweight-Network-Virtualization-Overview/">LNV</a> (Lightweight Network Virtualization) then transports the L2 bridges across an L3 fabric.</p>
<p><strong>Configurations</strong></p>
<p><strong>leaf01 Config</strong></pEthernet-Virtual-Private-Network---EVPN">EVPN</a> for both VLAN and VXLAN bridging as well as VXLAN and external routing.</p>
...
<pre><code>/etc/network/interfaces</code></p>
<pre><code>auto lo
iface lo inet loopback 
  address 10.0.0.11/32
  vxrd-src-ip 10.0.0.11
  vxrd-svcnode-# Loopback interface
auto lo
iface lo inet loopback
  address 10.0.0.11/32
...
iface vni13
  mtu 9000
  vxlan-id 13
  vxlan-local-tunnelip 10.10.10.101
  clagd-vxlan-anycast-bridge-access 13
  bridge-learning off
...
iface vni24
  mtu 9000
  vxlan-id 24
  vxlan-local-tunnelip 3610.0.0.11
 
auto vni-10
iface vni-10 
  bridge-access 24
...
<p><strong>leaf02 Config</strong    bridge-learning off
    bridge-access 4001
...
<p><code>summary>Leaf02 /etc/network/interfaces </code></psummary>
<pre><code># Loopback interface
auto lo
iface lo inet loopback 
  address 10.0.0.12/32
  Vxrd-srcclagd-vxlan-anycast-ip 10.0.0.112
...
<p><strong><strong>Caveats</strong></strong></p>
<ul>
<li><p>Needs MLAG (with the same caveats from the <a href="#src-83629916715_DataCenterHosttoToRArchitecture-mlag">MLAG section</a> above) and <a href="/version/cumulus-linux-377/Layer-2/Spanning-Tree-and-Rapid-Spanning-Tree">spanning tree</a></p></li>
</ul></td>
</tr>
</tbody>
</table>
...
<li><p><a href="/version/cumulus-linux-37740/Network-Virtualization/Lightweight-Network-Virtualization-Overview/">CEthernet-Virtual-Private-Network---EVPN">Ethernet Virtual Private Network - EVPN</a></p></li>
<li><p><a href="/version/cumulus L-linux Lightweight -40/Network -Virtualization (LNV) documentation/VXLAN-Routing">VXLAN Routing</a></p></li>
</ul></td>
</tr>
</tbody>
</table>
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc5Njk4NzUxN119
-->