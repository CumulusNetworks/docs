...
package contains support for IPv6, the `src_ip` option described below,
...
1.  Add the hostname or IP address of at least one RADIUS server (such
    ...
    `/etc/hosts` file manually. However, this can cause problems since
...
4.  Set the `vrf-name` field. This is typically set to *mgmt* if you are
    using a [management
    VRF](/version/cumulus-linux-40/Layer-3/Management-VRF). You cannot
...
In this example, the `ops` user is not a privileged RADIUS user so they
...
## <span id="src-8366280_RADIUSAAA-remove" class="confluence-anchor-link"></span><span>Remove RADIUS Client Packages</span>
...
## <span>Related Information</span>
  ...
  - [TACACS+
    client](/version/cumulus-linux-40/System-Configuration/Authentication-Authorization-and-Accounting/TACACS-Plus)
