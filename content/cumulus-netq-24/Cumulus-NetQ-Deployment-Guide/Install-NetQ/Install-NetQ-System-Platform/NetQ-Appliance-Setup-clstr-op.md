...
4. Run the Bootstrap CLI. Be sure to replace the *eno1* interface used in this example with the interface or IP address on the appliance used to listen for NetQ Agents.
    ...
    {{<netq-install/bootstrap server="single" version="2.4.1" platform="appliance" deployment="onprem">}}
...
6. Configure the IP address, hostname, and password using the same steps as as for the master node. Refer to {{<link title="#NetQ-Appliance-Setup-clstr-op" text="Configure the Password, Hostname and IP Address">}}.
...
10. Run the Bootstrap CLI on the worker node.
    ...
    {{<netq-install/bootstrap server="cluster" version="2.4.1" deployment="onprem" platform="appliance">}}
