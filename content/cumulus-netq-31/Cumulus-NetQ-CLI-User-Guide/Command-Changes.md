...
| netq add trace &lt;mac> [vlan &lt;1-4096>] from (&lt;src-hostname> \| &lt;ip-src>) [vrf &lt;vrf>] [alert-on-failure] | Adds a layer 2 trace for a MAC address from a hostname or IP address. The trace results appear in the NetQ UI only, not the terminal. | 3.1.0 |
| netq add trace name &lt;text-new-trace-name> &lt;mac> [vlan &lt;1-4096>] from (&lt;src-hostname> \| &lt;ip-src>) [vrf &lt;vrf>] interval &lt;text-time-min> [alert-on-failure] | Adds a layer scheduled 2 trace to be run at a given interval for a MAC address from a hostname or IP address. The trace results appear in the NetQ UI only, not the terminal. | 3.1.0 |
| netq add trace &lt;ip> from (&lt;src-hostname> \| &lt;ip-src>) [vrf &lt;vrf>] [alert-on-failure] | Adds a layer 3 trace for an IP address from a hostname or IP address. The trace results appear in the NetQ UI only, not the terminal. | 3.1.0 |
