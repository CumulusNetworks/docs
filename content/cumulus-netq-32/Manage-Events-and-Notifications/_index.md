...
- **Threshold-based** (TCA): selected set of events generated based on ser-configured threshold values
...
The NetQ CLI provides the `netq show events` command to view system events for a given time frame. The `netq show wjh-drop` command lists all WJH events or those with a selected drop type.
