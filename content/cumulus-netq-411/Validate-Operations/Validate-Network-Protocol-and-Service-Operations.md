...
To run on-demand validations with the CLI, use the {{<link title="check" text="netq check">}} commands. You can create filters that suppress validation failures based on hostnames, failure reason, and other parameters with the {{<link title="add/#netq-add-check-filter" text="netq add check filter">}} command.
