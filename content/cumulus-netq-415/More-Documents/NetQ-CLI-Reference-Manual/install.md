...
netq install nvl bundle <text-bundle-url> 
    ...
    <text-cluster-config>
    [ipv6] 
    [wg-ipv6-addr <text-ipv6-addr-01> <text-ipv6-addr-02> <text-ipv6-addr-03>] 
    [wg-listen-port <text-wg-listen-port>]
    [ignore-pre-checks]
```
...
| Option | Value | Description |
| ---- | ---- | ---- |
| ipv6| NA | Installs NetQ NVLink using IPv6 addresses |
| wg-ipv6-addr | \<text-ipv6-addr-01\> | Specify the IPv6 addresses for WireGuard tunneling |
| wg-listen-port | \<text-wg-listen-port\> | Sets the UDP port for the WireGuard service (IPv6 installations only) |
