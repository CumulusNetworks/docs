...
```
2. Run `netq bootstrap reset purge-db` on each of the three nodes in your cluster.
...
```
nvidia@<hostname>:~$ netq install nvl bundle /mnt/installables/NetQ-5.2.1.tgz kong-rw-password <rw-password> kong-ro-password <ro-password> /home/nvidia/nvl-cluster-config.json
```
