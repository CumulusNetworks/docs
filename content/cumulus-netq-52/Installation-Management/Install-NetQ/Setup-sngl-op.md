...
1. Download the NetQ image.
    ...
    e. For deployments using KVM, download the **NetQ SW 5.2.1 KVM** image. For deployments using VMware, download the **NetQ SW 5.2.1 VMware** image.<br>
...
{{<netq-install/vm-setup hypervisor="kvm" deployment="single-server" version="5.2">}}
...
```
nvidia@hostname:~$ netq install standalone full ip-addr <ip-address> bundle /mnt/installables/NetQ-5.2.1.tgz
```
...
<div class="notices note"><p></p><p>NetQ uses the 10.244.0.0/16 (<code>pod-ip-range</code>) and 10.96.0.0/16 (<code>service-ip-range</code>) networks for internal communication by default. If you are using these networks, you must override each range by specifying new subnets for these parameters in the install command:</p>
    <pre><div class="copy-code-img"></div>nvidia@hostname:~$ netq install standalone full interface eth0 bundle /mnt/installables/NetQ-5.2.1.tgz pod-ip-range &lt;pod-ip-range&gt; service-ip-range &lt;service-ip-range&gt;</pre>
    <p></p></div>
...
```
nvidia@hostname:~$ netq install standalone full interface eth0 bundle /mnt/installables/NetQ-5.2.1.tgz restore /home/nvidia/backup-netq-standalone-onprem-5.0.0-2025-12-11_19_50_12_UTC.tar
```

Example using IP address:

```
nvidia@hostname:~$ netq install standalone full ip-addr <ip-address> bundle /mnt/installables/NetQ-5.2.1.tgz restore /home/nvidia/backup-netq-standalone-onprem-5.0.0-2025-12-11_19_50_12_UTC.tar
```
