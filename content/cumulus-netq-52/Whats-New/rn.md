...
| 4784336 | The NetQ for NVLink deployment option is not supported in air-gapped environments. | 5.0.0-5.1.0, 5.2.1 | |
| 4687477 | When you run a validation against a group of devices with specific labels, NetQ ignores any pre-configured filters. | 5.0.0-5.1.0, 5.2.1 | |
| 4682275 | NVLink cluster installations do not validate that each node has a unique hostname. If two nodes share a common hostname, NetQ does not flag the issue after the installation completes. | 5.0.0-5.1.0, 5.2.1 | |
| 4681581 | The <code>netq bootstrap reset purge-db</code> command might take up to 60 minutes to complete on Base Command Manager scale deployments. | 5.0.0-5.1.0, 5.2.1 | |
| 4399074 | When connecting a switch to NMX-T or NMX-C through the service registration workflow, use either the IP address or the hostname. Using both creates duplicate registrations, and the operation does not fail as expected. | 5.0.0-5.1.0, 5.2.1 | |
| 4389662 | When a cluster installation fails a cluster VIP validation check, the installer generates an opta-support archive and prompts you to send it to NVIDIA support instead of prompting you to fix the initial error. | 4.15.0-4.15.1, 5.0.0-5.1.0, 5.2.1 | |
| 4122430 | When the master node is unreachable, a worker node might report the output of <code>netq show status</code> as <code>Not Installed</code> instead of indicating that the cluster was degraded. | 4.12.0-4.15.1, 5.0.0-5.1.0, 5.2.1 | |
| 4100882, 4119697 | When you attempt to export a file that is larger than 200MB, your browser might crash or otherwise prevent you from exporting the file. To work around this issue, use filters in the UI to decrease the size of the dataset that you intend to export. | 4.12.0-4.15.1, 5.0.0-5.1.0, 5.2.1 | |
