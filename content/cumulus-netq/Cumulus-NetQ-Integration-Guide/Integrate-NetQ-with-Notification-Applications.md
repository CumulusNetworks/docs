...
NetQ supports threshold-based events, also called threshold crossing alerts (TCA), which are triggered based on a user-defined limit. The simplest configuration you can create is one that sends a TCA event generated by all devices and all interfaces to a single notification application. Use the `netq add tca` command to configure the event. Its syntax is:
...
Several utilization, traffic volume, and sensor releated events are supported. These support a maximum threshold option. These include:
...
```
netq add tca event_id TCA_CPU_UTILIZATION_UPPER scope * threshold 95 channel tca_slack_ifstats
```
...
```
netq add tca event_id TCA_TXBYTES_UPPER scope leaf12,* threshold 20000 channel tca_pd_ifstats
```
...
```
netq add tca event_id TCA_SENSOR_TEMPERATURE_UPPER scope leaf12,temp1 threshold 32 channel syslog-netq
```
...
```
netq add tca event_id TCA_CPU_UTILIZATION_UPPER scope * severity critical threshold 95 channel tca_slack_resources
```

Or if an event is important, but not critical. Set the `severity` to *info*:

```
netq add tca event_id TCA_TXBYTES_UPPER scope leaf12,* severity info  threshold 20000 channel tca_pd_ifstats
```
...
```
netq add tca event_id TCA_SENSOR_TEMPERATURE_UPPER scope leaf*,temp1  threshold 32 channel syslog-netq

netq add tca event_id TCA_SENSOR_TEMPERATURE_UPPER scope *,temp1  threshold 32 channel tca_sensors,tca_pd_sensors

netq add tca event_id TCA_SENSOR_TEMPERATURE_UPPER scope leaf03,temp1  threshold 29 channel syslog-netq
```
