...
RX error counters can be seen in the output of `cl-netstat` as `RX_ERR`, as shown below.
...
RX error counters can be seen in the output of `ip -s link show`, as shown below.
...
These RX\_ERR or HwIfInErrors indicate some Ethernet data frames are being corrupted somewhere along the transmission line, typically due to some bad cable or transceiver. These errors may be detected as part of the cyclic redundancy check (CRC) algorithm in the Frame Check Sequences (FCS) calculation.
...
You need to configure the switches in the data path, particularly:
