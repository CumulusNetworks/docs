...
Configures BGP conditional disaggregation to advertise specific prefixes when a failure is detected, while continuing to advertise the aggregate route. Combined with anycast Site-of-Origin (SOO) matching, BGP conditional disaggregation triggers peer plane leafs to conditionally originate specific routes to draw traffic instead of using the aggregate route that might lead to an unreachable destination.
...
Configures BGP conditional disaggregation for IPv6 to advertise specific prefixes when a failure is detected, while continuing to advertise the aggregate route. Combined with anycast Site-of-Origin (SOO) matching, BGP conditional disaggregation triggers peer plane leafs to conditionally originate specific routes to draw traffic instead of using the aggregate route that might lead to an unreachable destination.
...
Enables and disables BGP for IPv6 for the specified VRF. The default setting is `enable off`.
...
## <h>nnv set vrf \<vrf-id\> router bgp soo-source</h>

Configures PIC in a multiplane topology by setting the SOO source IP address on a leaf. Configuring the same SOO source IP address on multiple leaf switches puts them in the same anycast group.
