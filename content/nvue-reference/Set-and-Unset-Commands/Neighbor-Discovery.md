...
## <h>nv set interface <interface-id> neighbor ipv6 \<address\> lladdr \<mac-address\></h>
...
| `<IP6-address>` |  The static IPv6 address. |
| `<mac-address>` |  The MAC address you want to associate with IPv6 address. |
...
## <h>nv set interface <interface-id> neighbor ipv6 \<address\> lladdr \<mac-address\> flag</h>
...
## <h>nv set interface <interface-id> neighbor ipv6 \<address\> lladdr \<mac-address\> state</h>

Configures the sate of the neighbor in the IP neighbor table (`delay`, `failed`, `incomplete`, `noarp`, `permanent`, `probe`, `reachable`, or `stale`).
