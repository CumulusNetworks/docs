...
## <h>nv show system dot1x pre-auth</h>

Shows if the switch allows LLDP packets on unauthorized 802.1X ports on ingress, egress, or both.
...
```
cumulus@switch:~$ nv show system dot1x pre-auth
                operational  applied
--------------  -----------  -------
allow-protocol                      
  lldp                       ingress
...
## <h>nv show system dot1x pre-auth allow-protocol</h>

Shows which protocols are allowed on unauthorized 802.1X ports.
...
```
cumulus@switch:~$ nv show system dot1x pre-auth allow-protocol
      operational  applied
----  -----------  -------
lldp  none         none
```
...
## <h>nv show system dot1x tx-identity-request</h>

Shows 802.1x reauthentication configuration. 802.1X reauthentication ensures that the switch revalidates connected clients for authorization and security after a port flap or switch reboot.
