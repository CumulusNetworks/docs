...
interfaces. Keep in mind the following for a VRF table:
  ...
  - Names for VRF tables can be up to 15 characters. However, you
    **cannot** use the name *mgmt*, as this name can **only** be used
    for [management VRF](/version/cumulus-linux-332/Layer-Three/Management-VRF).
...
`vrf task exec <vrf-name> <command>`. For example, to SSH from the 
switch to a device accessible through VRF *red*:
...
[Quagga](/version/cumulus-linux-332/Layer-Three/Quagga-Overview) learns
...
[BGP unnumbered interface configurations](/version/cumulus-linux-332/Layer-Three/Border-Gateway-Protocol-BGP/#using-bgp-unnumbered-interfaces)
...
## Caveats and Errata
  ...
  - You cannot configure [EVPN address families](/version/cumulus-linux-332/Network-Virtualization/Ethernet-Virtual-Private-Network-EVPN)
    within a VRF.
