...
technology such as [Lightweight Network Virtualization (LNV)](/cumulus-linux-35/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/)
in Cumulus Linux.
...
plane for [VXLAN](/cumulus-linux-35/Network-Virtualization/)
...
features include:
  ...
  - Support for dual-attached hosts via 
    [VXLAN active-active mode](/cumulus-linux-35/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/LNV-VXLAN-Active-Active-Mode).
    MAC synchronization between the peer switches is done using
    [MLAG](/cumulus-linux-35/Layer-1-and-2/Multi-Chassis-Link-Aggregation-MLAG).
...
about FRR, refer to the [FRR overview](/cumulus-linux-35/Layer-3/FRRouting-Overview/).
...
needed in
[FRRouting](/cumulus-linux-35/Layer-3/FRRouting-Overview/).
...
For a bridge in [traditional mode](/cumulus-linux-35/Layer-1-and-2/Ethernet-Bridging-VLANs/Traditional-Mode-Bridges),
...
[Head End Replication (HER)](/cumulus-linux-35/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/#head-end-replication).
HER is enabled by default, as it is when 
[Lightweight Network Virtualization (LNV)](/cumulus-linux-35/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/)
is used.
...
functions with the 
[Spectrum A1 chip](https://support.cumulusnetworks.com/hc/en-us/articles/115015646107).
...
The active-active configuration should include the following:
  ...
  - The `clagd-vxlan-anycast-ip` parameter must be under the 
    [loopback stanza](/cumulus-linux-332/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/LNV-VXLAN-Active-Active-Mode/#configuring-the-anycast-ip-address)
    on both peers
  - The anycast address should be advertised to the routed fabric from
    both peers
  - The
    [VNIs](/cumulus-linux-332/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/LNV-VXLAN-Active-Active-Mode/#example-vxlan-active-active-configuration)
  ...
  - The
    [peerlink](/cumulus-linux-332/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/LNV-VXLAN-Active-Active-Mode/#example-vxlan-active-active-configuration)
...
([virtual routing and forwarding](/cumulus-linux-35/Layer-3/Virtual-Routing-and-Forwarding-VRF)).
...
When configuring [VXLAN routing](/cumulus-linux-35/Network-Virtualization/VXLAN-Routing),
...
1.  Enable advertisement of EVPN prefix (type-5) routes. Refer to the
    [next section](#prefix-based-routing-evpn-type-5-routes) for
...
You can examine MAC addresses for all VNIs using 
...
You can examine neighbor entries for all VNIs using 
...
corresponding to all remote VTEPs by running 
`net show evpn rmac vni <vni>`. This command is only relevant for an L3-VNI:
...
You can examine router MACs for all L3-VNIs using 
...
You can examine gateway next hops for all L3-VNIs using 
...
You can examine the VRF routing table by running 
`net show route vrf <vrf-name>`. This command is not specific to EVPN. 
In the context of
...
`net show bgp l2vpn evpn route rd <rd-value>` command. This displays all 
...
`net show bgp vrf <vrf-name> ipv4 unicast` (or 
`net show bgp vrf <vrf-name> ipv6 unicast`):
...
The following caveats apply to EVPN in this version of Cumulus Linux:
  ...
  - In a [VXLAN active-active](/cumulus-linux-35/Network-Virtualization/Lightweight-Network-Virtualization-LNV-Overview/LNV-VXLAN-Active-Active-Mode)
  ...
  - Currently, only switches with the Mellanox Spectrum chipset or the
    ...
    please [read release note 766](https://support.cumulusnetworks.com/hc/en-us/articles/115015543848#rn766)
