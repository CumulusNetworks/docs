...
weight: 327
aliases:
 - /display/CL40/Hardware-enabled-DDOS-Protection
...
---
It is crucial to protect the control plane on the switch to ensure that
the proper control plane applications have access to the CPU. Failure to
do so increases vulnerabilities to a Denial of Service (DOS) attack.
Cumulus Linux provides control plane protection by default. It also
offers a DDOS protection mechanism, which protects data plane, control
plane and management plane traffic in the switch. It drops any packets
that match one or more of the following criteria while incurring no
performance impact:
...
Cumulus Networks recommends enabling DDOS protection when deploying a
...
3.  Open the
    ...
    file in a text editor. Set the following checks to *true*, then save
    the file:
    ...
    echo](/version/cumulus-linux-40/Layer-3/Bidirectional-Forwarding-Detection---BFD)
    function. For example, if you enable `dos.udp_ports_eq`, all the BFD
    packets are dropped because the BFD protocol uses the same source
