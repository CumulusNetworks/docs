...
weight: 25
aliases:
 - /display/CL40/Monitoring-and-Troubleshooting
...
The serial console is a useful tool for debugging issues, especially
when you find yourself rebooting the switch often or if you do not have
...
inaccessible via the console. Review `grub` changes carefully before you
implement them.
...
File](/version/cumulus-linux-40/Monitoring-and-Troubleshooting/Understanding-the-cl-support-Output-File/).

Run `cl-support` before you submit a support request to Cumulus Networks
...
## <span id="src-8366313_MonitoringandTroubleshooting-syslog_server" class="confluence-anchor-link"></span><span>Send Log Files to a syslog Server</span>
...
Enabled](#src-8366313_MonitoringandTroubleshooting-mgmtvrf) below.
...
There are applications in Cumulus Linux that can write directly to a log
...
them to files in the `/var/log` directory. There are default rules in
the `/etc/rsyslog.d/` directory that define where the logs are written:
...
| 20-clagd.conf     | Logs `clagd` messages to `/var/log/clagd.log` for [MLAG](/version/cumulus-linux-40/Layer-2/Multi-Chassis-Link-Aggregation---MLAG).                                       |
...
| 30-ptmd.conf      | Logs `ptmd` messages to `/var/log/ptmd.log` for [Prescription Topology Manager](/version/cumulus-linux-40/Layer-1-and-Switch-Ports/Prescriptive-Topology-Manager---PTM). |
| 35-rdnbrd.conf    | Logs `rdnbrd` messages to `/var/log/rdnbrd.log` for [redistribute neighbor](/version/cumulus-linux-40/Layer-3/Redistribute-Neighbor).                                    |
| 40-netd.conf      | Logs `netd` messages to `/var/log/netd.log` for [NCLU](/version/cumulus-linux-40/System-Configuration/Network-Command-Line-Utility---NCLU).                              |
...
1.  Create a file in `/etc/rsyslog.d/`. Make sure the filename starts
    with a number lower than 99 so that it executes before log messages
    are dropped in, such as `20-clagd.conf` or `25-switchd.conf`. The
    example file below is called `/etc/rsyslog.d/11-remotesyslog.conf`.
    ...
    *@@192.168.1.2:514*.
    ...
    only the messages in `/var/syslog` but not those in
    `/var/log/clagd.log` or `/var/log/switchd.log`, then name the file
    `98-remotesyslog.conf;` the number in the filename is lower than
    `/var/syslog` file `99-syslog.conf`.
...
### <span id="src-8366313_MonitoringandTroubleshooting-mgmtvrf" class="confluence-anchor-link"></span><span>Write to syslog with Management VRF Enabled</span>

You can write to syslog with [management
VRF](/version/cumulus-linux-40/Layer-3/Management-VRF) enabled by
...
burst values to rate-limit messages to the appropriate levels required
by your environment. For more information, read the [rsyslog
...
The following message is logged to `/var/log/syslog` when you run
...
`systemd` attempts to change group attributes that are read only. The
