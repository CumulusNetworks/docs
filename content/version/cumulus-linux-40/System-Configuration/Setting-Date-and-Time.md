...
weight: 63
aliases:
 - /display/CL40/Setting-Date-and-Time
...
### <span>Configure NTP Servers</span>
...
daemon, run the `ntpd -q` command. This command is the same as
`ntpdate`, which is to be retired and no longer available.
...
By default, the source interface that NTP uses is eth0. To change the
source interface:
...
the DHCP-generated file is named `/run/ntp.conf.dhcp`. This file is
...
the future, you can copy your current `/etc/ntp.conf` file to the
...
## <span id="src-8366266_SettingDateandTime-PTP" class="confluence-anchor-link"></span><span>Precision Time Protocol (PTP) Boundary Clock</span>
