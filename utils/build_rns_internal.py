...
Internal file naming conventions (flat in the root of the server):
  ...
  Release list  : release_notes_and_license_list.json
'''
...
def get_products():
    '''
    Download the engineering-provided JSON file from the internal web server,
    ...
    '''
    session = requests.Session()
    url = "{}release_notes_and_license_list.json".format(INTERNAL_BASE_URL)
    response = session.get(url)
    if response.status_code != 200:
...
def main():
    ...
    print("Fetching product and version list from {}".format(INTERNAL_BASE_URL))
