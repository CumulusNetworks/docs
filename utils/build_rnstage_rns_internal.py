#!/usr/bin/env python3
'''
This script will pull the collection of release note JSON files from the internal
web server at http://penta-02.lab2300.labs.mlnx/users/release-notes/

This is the STAGE variant — it reads the stage release list
(release_notes_and_license_list_stage.json) instead of the production list.
...
Internal file naming conventions (flat in the root of the server):
  Cumulus Linux : CM-<version>-fixed.json / CM-<version>-affects.json
  NetQ          : NETQ-<version>-fixed.json / NETQ-<version>-affects.json
  Release list  : release_notes_and_license_list_stage.json
'''
...
# ---------------------------------------------------------------------------
# Global knobs — set to "YES" to build that product, "NO" to skip it.
# ---------------------------------------------------------------------------
BUILD_CL   = "YES"   # Build Cumulus Linux (CM-*) release notes
BUILD_NETQ = "NO"    # Build NetQ (NETQ-*) release notes

# ---------------------------------------------------------------------------
# Internal server settings
# ---------------------------------------------------------------------------
INTERNAL_BASE_URL = "http://penta-02.lab2300.labs.mlnx/users/release-notes/"

# Stage release list filename (differs from the production script).
RELEASE_LIST_FILENAME = "release_notes_and_license_list_stage.json"

# Prefix used in filenames for each product short name.
_PRODUCT_FILE_PREFIX = {
    "cl":   "CM",
    "netq": "NETQ",
}

# Accumulates (url, status_code) tuples for every file that could not be fetched.
# Populated by get_json(); printed as a summary at the end of main().
_download_failures = []


def get_json(product, version, json_file):
    '''
    Downloads the json file for a given product/version from the internal web server.
    Also sort based on "ticket" value.

    product   - either "cl" or "netq"
    version   - the specific version of file to download. For example, "3.2.1" or "5.19.0"
    json_file - either "fixed" or "affects"

    Internal filename format:
      cl   -> CM-<version>-<json_file>.json      e.g. CM-5.19.0-affects.json
      netq -> NETQ-<version>-<json_file>.json    e.g. NETQ-5.3.0-affects.json

    Returns an empty list and logs a warning if the file cannot be fetched, so
    the build continues rather than aborting on a missing version.
    '''
    prefix = _PRODUCT_FILE_PREFIX.get(product)
    if prefix is None:
        print("Unknown product {}".format(product))
        exit(1)

    filename = "{}-{}-{}.json".format(prefix, version, json_file)
    url = "{}{}".format(INTERNAL_BASE_URL, filename)

    session = requests.Session()
    response = session.get(url)
    if response.status_code != 200:
        msg = 'WARNING: Unable to download {}. Received response {} — skipping.'.format(
            url, response.status_code)
        print(msg)
        _download_failures.append((url, response.status_code))
        return []
...
EXCLUDED_VERSIONS = {
    ...
    "netq": ["2.4.0", "2.4.1", "3.0.0", "3.1.0", "3.2.0", "3.2.1", "3.3.0", "3.3.1", "4.0.0", "4.0.1", "4.1.0", "4.1.1", "4.2.0", "4.3.0", "4.4.0", "4.4.1", "4.5.0", "4.6.0", "4.7.0", "4.8.0", "5.0.0", "5.0.1", "5.1.0", "5.2.0", "5.2.1", "5.3.0"]
}
...
# Note: cl is intentionally empty on stage (no hidden CL versions).
HIDDEN_VERSIONS = {
    "cl": [],
    "netq": ["5.2.0"],
}
...
def _greatest_version_strictly_below_e(cursor, e, hi, literals, canonical_releases):
    ...
    Prefer actual shipping releases from release_notes_and_license_list_stage.json
...
def _split_range_removing_hidden(lo, hi, hidden_set, literals, canonical_releases):
    ...
    release_notes in release_notes_and_license_list_stage.json); advancing past a
...
def filter_version_list_for_display(
    ...
    product from release_notes in release_notes_and_license_list_stage.json; when set,
...
def get_products():
    '''
    Download the engineering-provided stage JSON file from the internal web server,
    detailing the list of products and releases.
    ...
    versions from this file only (see release_notes_and_license_list_stage.json).
    '''
    session = requests.Session()
    url = "{}{}".format(INTERNAL_BASE_URL, RELEASE_LIST_FILENAME)
    response = session.get(url)
    if response.status_code != 200:
...
def main():
    ...
    Products are gated by the BUILD_CL and BUILD_NETQ global knobs at the top of
    this file.  Set either to "NO" to skip that product entirely (no downloads,
    no markdown/XLS output).
    """

    # Map product short names to their controlling knob.
    product_knobs = {
        "cl":   BUILD_CL,
        "netq": BUILD_NETQ,
    }

    print("Fetching stage product and version list from {}{}".format(
        INTERNAL_BASE_URL, RELEASE_LIST_FILENAME))
    products = get_products()

    for product in products:
        knob = product_knobs.get(product, "NO")
        if knob.strip().upper() != "YES":
            print("Skipping {} (BUILD_{} = {})".format(
                product_string(product), product.upper(), knob))
            continue
    ...
    if _download_failures:
        print("\n{} file(s) could not be downloaded and were skipped:".format(
            len(_download_failures)))
        for url, code in _download_failures:
